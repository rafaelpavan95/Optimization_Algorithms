...
while(iteracoes<maxi)
    ...
    for i=1:length(sk)
     ...
     dxk = -inv(transpose(A)*inv(Sk*Sk)*A)*(gradientevpa);
    ...
    if iteracoes ~= 1
        ...
        if max(abs(sk-sant)) <= episolon3 & max(abs(wk-want)) <= episolon3
   ...
   bk2 = -transpose(dxk)*(gradientevpa)/(transpose(dxk)*hessianavpa*dxk);
